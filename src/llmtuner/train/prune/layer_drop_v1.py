...
from transformers import LlamaForCausalLM, AutoConfig, MistralForCausalLM
...
def post_layers_drop(prune_model_save_path, target_layer, model, tokenizer, reserved_layer_list, accelerator: Accelerator, only_update_config=False):
    ...
    if accelerator.is_main_process:
        ...
        if target_layer == 'mlp':
            if not hasattr(out_cfg, 'drop_mlp_list') or out_cfg.drop_mlp_list == None:
                out_cfg.drop_mlp_list = dropped_layer_list
            else:
                out_cfg.drop_mlp_list = [idx for idx, v in enumerate(getattr(unwrapped_model.config, f'drop_{target_layer}_list', [])) if v] + dropped_layer_list
            out_cfg.drop_attn_list = None
        elif target_layer == 'attn':
            if not hasattr(out_cfg, 'drop_attn_list') or out_cfg.drop_attn_list == None:
                out_cfg.drop_attn_list = dropped_layer_list
            else:
                out_cfg.drop_attn_list = [idx for idx, v in enumerate(getattr(unwrapped_model.config, f'drop_{target_layer}_list', [])) if v] + dropped_layer_list
            out_cfg.drop_mlp_list = None
        else:
            raise ValueError("Unsupported target_layer!")
        
        accelerator.print(f"Dropped {target_layer} list: {dropped_layer_list}")
